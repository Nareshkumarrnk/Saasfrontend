...
import { sendFallbackWelcomeEmail } from "./fallback-email"
...
export async function sendWelcomeEmail(email: string, name: string) {
  ...
  if (process.env.RESEND_API_KEY) {
    try {
      ...
      console.error("Resend email failed, trying fallback:", error)
      // Fall back to nodemailer
      return await sendFallbackWelcomeEmail(email, name)
    }
  } else {
    // Use fallback email service
    console.log("RESEND_API_KEY not found, using fallback email service")
    return await sendFallbackWelcomeEmail(email, name)
  }
}
